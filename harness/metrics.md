...
| G7 | replay | `pytest -q && ruff check . && python demo.py` | full suite + demo clean after refactor |
| G8 | host #2 | `pytest -q tests/test_harness.py` | adapters/harness.py authors, resolves, calibrates |
| G9 | ship | `python -m build && ruff check .` | package builds, lint clean, changelog present |
